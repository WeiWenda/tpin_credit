...
object OracleDBUtil {
  ...
  val options = new JDBCOptions(Map("url"->url,"driver"->driver))
  ...
  def saveWholeVertex(vertexs: VertexRDD[WholeVertexAttr], sqlContext: SQLContext, dst: String = "WWD_XYCD_VERTEX") = {
    ...
    JdbcUtils.saveTable(vertexDataFrame,Option(schema), false, options)
  }
  ...
  def saveFinalScore(finalScore: Graph[ResultVertexAttr, ResultEdgeAttr], sqlContext: SparkSession,
    ...
    JdbcUtils.saveTable(vertexDataFrame,Option(schema1), false, options)
  }
