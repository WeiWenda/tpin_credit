...
import wwd.entity.{WholeVertexAttr, WholeEdgeAttr, VertexAttr, EdgeAttr}
...
object XYShowTools {
    ...
    def findFrequence(d: Seq[(VertexId, Seq[Long])], degree: Int = 2): Seq[(Long, Long)] = {
        val frequencies = d
        val result =
            for (i <- 0 until frequencies.length) yield
                for (j <- i + 1 until frequencies.length) yield {
                    ...
                    if (list1.intersect(list2).size >= degree)
    ...
    def getPath(graph: Graph[Paths, WholeEdgeAttr], weight: Double, maxIteratons: Int = Int.MaxValue, initLength: Int = 1, direction: Int = 1, justGD: Boolean = false) = {
        ...
        def sendPathsReverse(edge: EdgeContext[Paths, Int, Paths],
                             length: Int): Unit = {
            // 过滤掉仅含交易权重的边 以及非起点
    ...
    def addIL(graph: Graph[WholeVertexAttr, WholeEdgeAttr], weight: Double, degree: Int = 2) = {
        ...
        val messagesOfControls = getPath(initialGraph, weight, maxIteratons = 3).mapValues(lists => lists.filter(_.size > 1).map(_.head)).filter(_._2.size > 0)
