...
//    }
    ...
    def run(tpin: Graph[VertexAttr, EdgeAttr], sqlContext: SQLContext, bypass: Boolean = false, lambda: Int = 1) = {
        ...
        val belAndPl = tpin.mapTriplets { case triplet =>
            ...
            val bel = computeBel(triplet.attr.il_bl, triplet.attr.tz_bl, triplet.attr.kg_bl, triplet.attr.jy_bl)
            val pl = computePl(triplet.attr.il_bl, triplet.attr.tz_bl, triplet.attr.kg_bl, triplet.attr.jy_bl)
